...
Toolset.CRED.EditorPagePrototype.prototype.initMainEditor = function() {
    ...
    // When switching back to Visual editor, dialog may not be displayed if editor content hasn't changed.
    this.hasEditorChanged = false;
    this.editorInstance.on( 'change', function() {
        currentInstance.hasEditorChanged = true;
    } )
...
Toolset.CRED.EditorPagePrototype.prototype.addEvents = function() {
    ...
    jQuery( document ).on( 'change', '#cred-editor-expert-mode-switcher', function() {
        ...
        jQuery( '.js-cred-editor-notice-switcher' ).toggleClass( 'hidden' );
        currentInstance.hasEditorChanged = false;
    })
    ...
    jQuery( document ).on( 'click', '#cred-editor-expert-mode-switcher', function() {
        ...
        if ( !$this.is(':checked') && currentInstance.hasEditorChanged ) {
                ...
                buttons: [
                    ...
                    },
                    {
                        ...
                        click: function() {
                            ...
                            // If form was saved as HTML, scaffold is not generated yet
                            if ( ! jQuery('.cred-editor-scaffold-item-list').length ) {
                                Toolset.hooks.doAction( 'cred_editor_init_scaffold' );
                            }
                            ...
                            // Resets data and sidebar
                            jQuery( '#cred_post_type, [name="_cred[form][user_role][]"]' ).change();
                            // setTimeout needed due to css effects delay
