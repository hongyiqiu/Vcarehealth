...
if(isset($_POST['commentList']) && $_POST['action']=='select'){
	...
	if(!empty($_POST["search"])){
		$filter = trim(preg_replace('/\s*\([^)]*\)/', '', $_POST["search"]));
		$storefilter = " AND (lower(ssf_wp_store)  like  lower('%".trim(preg_replace('/\s*\([^)]*\)/', '', $_POST["search"]))."%') )";
		}
	else{
		$storefilter = '';
	}
...
$_POST['ssf_wp_user_email'] = trim(preg_replace('/\s*\([^)]*\)/', '', $_POST["ssf_wp_user_email"]));
 ...
 if ($check[0]["count"] <= 0) {
	...
	if ($new_loc_id > 0) {
	 ...
	 $ssf_conatct_email=(trim($ssf_wp_vars['ssf_conatct_email'])!="")? ssfParseToXML($ssf_wp_vars['ssf_conatct_email']) : "";
	  ...
	  if($ssf_conatct_email!=""){
		  
		  
		$storeName=$wpdb->get_results("SELECT ssf_wp_store FROM ".SSF_WP_TABLE." WHERE ssf_wp_id='".intval($_POST["ssf_wp_store_id"])."'", ARRAY_A);
		
		if(!empty($storeName)){
			foreach($storeName as $rowid)
			{
				$storenamemail=$rowid['ssf_wp_store'];

			}
		}
		
	  // print data 
	  // $ssfResponse['message'] = "Personal message".$storenamemail;
		  
	  $to = $ssf_conatct_email;
      $subject = 'Your store "'.$storenamemail.'" has received a new rating.';
      $body =  'Store name: '.$storenamemail.'<br/>';
	  $body .= 'Rating : '.$_POST["ssf_wp_ratings_score"].'<br/>';
	  $body .= 'Review : '.$_POST["ssf_wp_comment"].'<br/>';
...
$query=$wpdb->get_results("SELECT count(*) as count, AVG(ssf_wp_ratings_score) as score FROM ".SSF_WP_SOCIAL_TABLE." WHERE 1 AND ssf_wp_store_id = '".intval($_POST['ssf_wp_id'])."'", ARRAY_A);
$rattingView='<div class="reciewInfoStar"><a class="infoRatingPopUp" onclick="showCommentPopup();">';
	for($i=0; $i<5; $i++){
		if($i<round($query[0]["score"], 2)){
...
if(isset($_POST['action']) && $_POST['action']=='status'){
	$_POST['review_set'] = trim(preg_replace('/\s*\([^)]*\)/', '', $_POST["review_set"]));
