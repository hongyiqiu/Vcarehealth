...
<th style='/*background-color:#000;*/ vertical-align:middle; font-family:inherit; font-size:12px;'>$addStore<input class='button-primary' type='button' value='".__("Delete", SSF_WP_TEXT_DOMAIN)."' onclick=\"if(confirm('".__("Are you sure you want to remove the store[s)", SSF_WP_TEXT_DOMAIN)."?')){LF=document.forms['locationForm'];LF.act.value='delete';LF.submit();}else{return false;}\">
<button class='button-primary ssf-tooltip' type='button' onclick=\"if(confirm('".__("Are you sure you want to publish the store[s)", SSF_WP_TEXT_DOMAIN)."?')){ ssfPublishAll(); }else{return false;}\"><i class='fa fa-check' aria-hidden='true'></i>
<span class='ssf-tooltiptext ssf-tooltip-top'>Publish</span></button>
<button class='button-primary ssf-tooltip' type='button' onclick=\"if(confirm('".__("Are you sure you want to un publish the store[s)", SSF_WP_TEXT_DOMAIN)."?')){ ssfUnPublishAll(); }else{return false;}\"><i class='fa fa-times' aria-hidden='true'></i><span class='ssf-tooltiptext ssf-tooltip-top'>Unpublish</span></button>
</th>
";
...
print "<th style='width:20%; text-align:center; color:black; font-family:inherit; font-size:12px; {$mgmt_bgcolor} /**/' class='youhave'>";
