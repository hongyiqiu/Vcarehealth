...
if($tracker){
$ssf_wp_top_nav_hash[]='tracker';
$ssf_wp_top_nav_links[SSF_WP_TRACKER_PAGE]=__("<span class='fa fa-line-chart'>&nbsp;</span> Analytics", SSF_WP_TEXT_DOMAIN);
...
foreach ($ssf_wp_top_nav_links as $key=>$value) {
	...
	if (strpos($key, '__wp__/') !== false) {
		$key = str_replace("__wp__/","",$key);
	}
