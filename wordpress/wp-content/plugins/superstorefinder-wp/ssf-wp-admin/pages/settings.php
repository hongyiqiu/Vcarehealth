...
if (!empty($_POST['ssf_wp_map_settings'])) { 
	...
	global $ssf_wp_vars;
	    if(!empty($_FILES)){
         // isset Determines if a variable is set and is not NULL. Set Size Limit less then 10 MB=10485760 bytes. Extension must be png jpg.
			...
			$exp = explode(".",strtolower(trim($_FILES["custom_marker"]["name"])));
			$ext = end($exp);
          ...
          if(!empty($_FILES['custom_marker_active']['name'])){
			$valid_exts = array("jpg","jpeg","gif","png");
			$exp = explode(".",strtolower(trim($_FILES["custom_marker_active"]["name"])));
			$ext = end($exp);
			if(in_array($ext,$valid_exts)){
				$max_dimension = 800; // Max new width or height, can not exceed this value.
				 $dir=SSF_WP_UPLOADS_PATH."/images/icons/";
	...
	ssf_wp_data('ssf_wp_vars', 'update', $ssf_wp_vars);
		if(isset($_POST['data_source']) && $_POST['data_source']=='true'){
			ssf_generate_json();
		}
...
function ssf_wp_md_display($data, $input_zone, $template, $additional_classes = "") {
    ...
    foreach ($filtered_data as $key => $value) {
		...
		if($value['label']=="Search bar caption"){
			print "</div>";
			print "<div class='input_title ssfaccordion'>
						
						<h3><span class='fa fa-bookmark'>&nbsp;</span> Labels</h3>
						
						<div class='clearfix'></div>
					</div>";
					print "<div class='ssfsetingpanel'>";
		}
...
if($showregion==1){
	...
	if($showContact==1){
		...
		if($value['label']=="Rating validation message"){
			$showAddRequestForm=1;
			print "</div>";
		}
		if($showAddRequestForm==1){
			$showAddRequestForm=0;
		print "<div class='input_title ssfaccordion'>
				<h3><span class='fa fa-map-marker'>&nbsp;</span> Request Add Store </h3>
				<div class='clearfix'></div>
			</div>";
		print "<div class='ssfsetingpanel'>";
		}

       if($value['label']=="Submit Request"){
		   print "</div>";
	   }
...
var acc = document.getElementsByClassName("ssfaccordion");
var i;
