<?php
/*
Plugin Name: Toolset Twenty Seventeen Integration
Plugin URI: http://wp-types.com/
Description: Layouts Integration for WordPress 2017 Theme
 ...
 *
 */
define('TOOLSET_2017_VERSION', '1.0');
if( !defined('TOOLSET_INTEGRATION_PLUGIN_THEME_NAME') ){
    define('TOOLSET_INTEGRATION_PLUGIN_THEME_NAME','Twenty Seventeen');
}

class WPDDL_Layouts_2017_Loader {
		...
		/** @noinspection PhpUnusedParameterInspection */ $layouts_version, $integration_support_version )
	{
		...
		if( version_compare( $layouts_version, '1.4.5' ) !== -1 && $supported_integration_api_version == $integration_support_version ) {
			...
			$loader = WPDDL_2017_Integration::get_instance();
	...
	public function print_layouts_inactive_message() {
		printf( '<div class="error"><p>%s</p></div>', __( 'Toolset twenty seventeen integration plugin requires Layouts to be active.', 'ddl-layouts' ) );
	}
...
WPDDL_Layouts_2017_Loader::getInstance();